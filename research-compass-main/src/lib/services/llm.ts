...
export async function chatWithResearchContext(
  ...
  const systemPrompt = `You are "The Autonomous Research Scientist AI Assistant", a specialist scientific co-pilot.
You have deep, direct context over the active research project:
...
Formulated Hypotheses: ${projectContext.hypotheses.map((h) => `${h.label}: ${h.statement}`).join("; ")}

RULES:
1. Ground your answers directly in the provided research corpus and analyzed findings.
2. If asked to compare papers or explain gaps, cite specific paper titles and author names from the project.
3. Be scientifically precise, concise, and helpful.
4. Clearly distinguish established literature consensus from AI-generated exploratory hypotheses.`;
...
function getSimulatedChatResponse(
  ...
  if (q.includes("gap") || q.includes("limitation")) {
    ...
    return `Based on the ${context.papers.length} analyzed papers in this project, the most critical research gap is the **${topGap?.title || "Environmental Generalization Gap"}**.

**Why it matters:**
Most current models achieve >95% accuracy on clean, lab-curated benchmarks like PlantVillage, but experience severe performance drops in dynamic field conditions with complex backgrounds, direct solar glare, and leaf occlusions.
...
${context.papers.slice(0, 2).map((p) => `- *${p.title}* (${p.year})`).join("\n")}`;
  ...
  if (q.includes("hypothesis") || q.includes("feasible") || q.includes("best")) {
    ...
    return `The highest-ranked hypothesis in this project is **${topHyp?.label || "H1"}**:

> "${topHyp?.statement || "Lightweight Hybrid Conv-Attention for edge robustness"}"

- **Overall Score:** ${topHyp?.overall_score || 8.9}/10
- **Feasibility:** ${topHyp?.feasibility_score || 9.2}/10 (High - utilizes standard PyTorch edge quantization)
- **Novelty:** ${topHyp?.novelty_score || 8.8}/10
- **Key Advantage:** It directly addresses the tradeoff between high diagnostic accuracy and real-time edge processing constraints on handheld farmer devices.`;
  }

  if (q.includes("paper") || q.includes("compare") || q.includes("dataset")) {
    return `In this research corpus for *"${context.researchQuestion}"*, we have gathered ${context.papers.length} peer-reviewed publications.

**Top Publications Analyzed:**
${context.papers.slice(0, 3).map((p, i) => `${i + 1}. **${p.title}** (${p.year}) - *${p.venue}* (${p.citation_count} citations)`).join("\n")}

The common consensus is that modern attention backbones provide superior feature representations, but dataset diversity remains the primary bottleneck for real-world deployment.`;
  }

  return `Regarding your question about **"${context.researchQuestion}"**:

Our synthesized corpus of ${context.papers.length} papers highlights that advancing beyond simple classification requires addressing real-world field robustness, edge efficiency, and multi-disease co-infections.

You can explore specific facets in the **Research Gaps**, **Hypothesis Lab**, and **Experiment Designer** tabs in your workspace!`;
}
