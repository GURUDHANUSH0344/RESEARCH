              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                <div className="space-y-1">
                  ...
                  <p className="text-slate-900 font-medium leading-relaxed">
                    {Array.isArray(interpretation.key_findings) ? interpretation.key_findings.join(", ") : interpretation.summary}
                  </p>
                </div>

                <div className="space-y-1">
                  <span className="text-[10px] font-bold text-slate-500 uppercase">Scientific Implications</span>
                  <p className="text-teal-800 font-medium leading-relaxed">
                    {interpretation.scientific_implications || interpretation.baseline_comparison}
                  </p>
                </div>
              </div>
            </div>
              ...
              <table className="w-full text-left text-xs border-collapse font-mono">
                <thead>
                  <tr className="bg-slate-100/70 border-b border-slate-200 text-slate-700 font-bold text-[10px]">
                    {data.columns.map((h: string, i: number) => (
                      <th key={i} className="py-2.5 px-4">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 text-slate-700">
                  {data.rawRows.slice(0, 10).map((row: Record<string, unknown>, rIdx: number) => (
                    <tr key={rIdx} className="hover:bg-slate-50">
                      {data.columns.map((h: string, cIdx: number) => (
